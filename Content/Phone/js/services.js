...
.directive('lazyContainer', [
  function(){
    ...
    return {
      restrict: 'EA', controller: ['$scope', '$element', function($scope, $element){
        var elements = {};
        function onLoad(){
        ...
        function isVisible(elem){
          ...
          var xVisible, yVisible, offset = 50;
          if(elemRect.bottom + offset >= containerRect.top && elemRect.top - offset <= containerRect.bottom){
            yVisible = true;
          }
          if(elemRect.right + offset >= containerRect.left && elemRect.left - offset <= containerRect.right){
            xVisible = true;
          }
          ...
          iScope.$watch('lazySrc', function(){
            ...
            if(isVisible(iElement)){
              if (iScope.animateVisible) {
                iElement.css({ 'background-color': '#fff', 'opacity': 0 });
                // '-webkit-transition': 'opacity ' + speed, 'transition': 'opacity ' + speed
              }
              iElement.attr('src', iScope.lazySrc);
            }else{
              var uid = getUid(iElement);
              iElement.css({ 'background-color': '#fff', 'opacity': 0 });
              elements[uid] = { iElement: iElement, iScope: iScope };
            }
          });
...
.directive('lazySrc', [
  function(){
    return {
      restrict: 'A', require: '^lazyContainer',
